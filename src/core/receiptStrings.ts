import { InvoiceStrings, ReceiptStrings } from './receiptPdf';

/** Invoice labels per locale (same WinAnsi/CJK rules as receipts). */
export function invoiceStrings(locale: string): InvoiceStrings {
  const base = receiptStrings(locale);
  const lang = locale.toLowerCase().split(/[-_]/)[0];
  if (lang === 'cs') {
    return {
      ...base,
      invoice: 'FAKTURA - AI UTILIZACE',
      repository: 'Repozitar / model',
      subtotal: 'Mezisoucet',
      grandTotal: 'CELKEM K UHRADE',
      generatedBy: 'Vygenerovano rozsirenim Copilot Cost Lens pro VS Code. Neni danovy doklad.',
    };
  }
  if (lang === 'de') {
    return {
      ...base,
      invoice: 'RECHNUNG - AI-NUTZUNG',
      repository: 'Repository / Modell',
      subtotal: 'Zwischensumme',
      grandTotal: 'GESAMTBETRAG',
      generatedBy: 'Erstellt mit der VS-Code-Erweiterung Copilot Cost Lens. Kein Steuerbeleg.',
    };
  }
  return {
    ...base,
    invoice: 'INVOICE - AI UTILIZATION',
    repository: 'Repository / model',
    subtotal: 'Subtotal',
    grandTotal: 'GRAND TOTAL',
    generatedBy: 'Generated by the Copilot Cost Lens extension for VS Code. Not a tax document.',
  };
}
